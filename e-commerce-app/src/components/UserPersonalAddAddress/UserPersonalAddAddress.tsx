...
const UserPersonalAddAddress = (): JSX.Element => {
  ...
  const postalCodeInputHandler = (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    const errString = validatePostalCode(val, countries.find(c => c.code === country)?.name || '');
      ...
      <Box>
        <Grid container>
          ...
          <Grid item
            ...
            >
              {countries.map(country => (<MenuItem value={country.code}
                                                   key={country.code}>{country.name}</MenuItem>))}
            </Select>
          </Grid>
          <Grid item
                xs={12}
